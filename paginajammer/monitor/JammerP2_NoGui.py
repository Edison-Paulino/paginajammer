...
class JammerNoGUI(gr.top_block):
    def __init__(self):
        ...
        self.src.set_gain(73, 0)
        self.src.set_if_gain(73, 0)
        self.src.set_bb_gain(0, 0)
        self.src.set_bandwidth(28e6, 0)
        ...
        self.noise1 = analog.fastnoise_source_c(analog.GR_GAUSSIAN, 1, 0, 8192)
        self.noise2 = analog.fastnoise_source_c(analog.GR_GAUSSIAN, 1, 165, 8192)
        ...
        # FFT para análisis
        self.stream_to_vector = blocks.stream_to_vector(gr.sizeof_gr_complex, self.vector_size)
        self.fft_block = fft.fft_vcc(self.vector_size, True, window.blackmanharris(self.vector_size), True)
        self.mag_squared = blocks.complex_to_mag_squared(self.vector_size)
        self.nlog10 = blocks.nlog10_ff(5, self.vector_size, 0)
        ...
        self.connect(self.src, self.stream_to_vector)
        self.connect(self.stream_to_vector, self.fft_block)
        self.connect(self.fft_block, self.mag_squared)
        self.connect(self.mag_squared, self.nlog10)
        self.connect(self.nlog10, self.zeromq_pub)
